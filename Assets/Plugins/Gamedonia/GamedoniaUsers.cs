...
public class GamedoniaUsers
{
	...
	public static void LoginUserWithSessionToken(string session_token, Action<bool> callback) {
		...
		Gamedonia.RunCoroutine(
			GamedoniaRequest.post("/account/login",JsonMapper.ToJson(body),auth,null,null,
				delegate (bool success, object data) {								
					if (success) sessionToken = JsonMapper.ToObject<GDSessionToken>((string)data);
					if (callback != null) callback(success);					
				}
		 	 )
		);
...
public class GamecenterAuthentication:IGamedoniaAuthentication {
	...
	void ProcessLogin(bool success) {		
		if (success) {
			...
			if (Gamedonia.INSTANCE.debug) Debug.Log("Gamedonia session couldn't be started!");
		}
	}
...
public class FacebookAuthentication:IGamedoniaAuthentication {
	...
	public void Authenticate(Action<bool> callback) {
		...
		if (!String.IsNullOrEmpty(_fb_uid) && !String.IsNullOrEmpty(_fb_access_token)) {
			...
			if (Gamedonia.INSTANCE.debug) Debug.Log("Facebook id or token not present impossible to perform login with it");
...
public class SessionTokenAuthentication:IGamedoniaAuthentication {
	...
	public void Authenticate(Action<bool> callback) {
		...
		if (!String.IsNullOrEmpty(session_token)) {
			GamedoniaUsers.LoginUserWithSessionToken(session_token, ProcessLogin);
		}else {
			if (Gamedonia.INSTANCE.debug) Debug.Log("Session Token not present impossible to perform login with it");
